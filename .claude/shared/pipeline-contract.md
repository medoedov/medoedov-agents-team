...
| `/update-framework` | A downstream project with `.claude/` exists and the public framework mirror is explicitly in scope | Updated `.claude/` framework files, `.sync-framework-state.json`, and a regenerated Codex runtime when the project uses one | Dry-run or apply succeeds, preserved paths stay untouched, mirrored deletions stay within the update set, and no push or publish occurs | Return to the unchanged project or feature state, `/sync-codex`, or `/end-session` |
