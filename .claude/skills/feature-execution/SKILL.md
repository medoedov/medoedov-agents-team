...
0. Check `work/{feature}/logs/checkpoint.yml`:
   - `last_completed_wave > 0` → this is a resume after context compaction.
     ...
     - If `stall_state.active == true AND reset_at <= now` -> resume mode.
       Re-spawn teammates from `stall_state.pending_teammates`, resume from `stall_state.last_active_wave`.
...
3. For each task, spawn **teammate + reviewers** (if task has reviewers):
   ...
   **Teammate** — `subagent_type: "general-purpose"`, `model: "opus"`, `team_name: "{team}"`
   ...
   pre-idle check: Before going idle, ALWAYS read your inbox.
   ...
   **{reviewers_block}** — include only when task has reviewers (not `reviewers: none`):
   ...
   Round Synchronization (M2): When you have multiple reviewers, after sending diff:
   - Wait until ALL reviewers respond with their R(N) report before sending R(N+1) to anyone.
   - If a reviewer is silent >30 minutes on R(N) → escalate to team-lead: "reviewer {name} stuck on R(N)".
   - Team-lead will decide: wait longer / proceed without / spawn new reviewer.
...
Audit Wave tasks (Code Audit, Security Audit, Test Audit, and optionally Bug Hunt) have `reviewers: none` — each auditor teammate IS the review. Spawn them as standard teammates (general-purpose, opus), each loads its methodology skill.

**Bug Hunt** is added to the audit wave when the feature is size M/L OR has external API integrations (LLM providers, messaging platform, database, cache). If the tech-spec includes a Bug Hunt task — spawn it as a 4th parallel auditor.
...
After all 3 reports:
- All clean → proceed to Feature Acceptance Review
- Issues found → spawn a fixer teammate (ad-hoc, code-writing skill), assign the auditors who found issues as reviewers, standard review protocol (max 3 rounds). After approval → proceed to Feature Acceptance Review. If unresolved after 3 rounds → escalate (see Escalation).

### Feature Acceptance Review

After Audit Wave passes (all clean or fixes applied), before Final Wave — run 3 parallel checks:

**1. Spec-vs-Reality check** — spawn `general-purpose` agent (sonnet):
```
Read user-spec.md acceptance criteria. For EACH criterion:
- Find the code/test that implements it
- Verify it actually works as specified (not just "code exists")
- Check edge cases mentioned in user-spec are handled
Report: list of criteria with pass/fail and evidence.
Write to: {feature_dir}/logs/working/acceptance-review.json
```

**2. UX Journey walkthrough** — spawn `ux-designer` agent:
```
Read user-spec.md. Walk through the COMPLETE user journey from start to finish:
- What does user see/do at each step?
- Is the flow intuitive? Any dead ends?
- Are error messages clear?
- Does the feature integrate naturally with existing bot UX?
Report: UX findings (if any) to {feature_dir}/logs/working/ux-review.json
```

**3. Regression scan** — spawn `bug-hunter` agent:
```
Focus NOT on new code, but on ADJACENT code that might break:
- What existing handlers/functions share state with the new feature?
- Do existing tests still describe correct behavior?
- Are there shared utilities (split_message, rate_limit, memory) that changed?
Report: regression risks to {feature_dir}/logs/working/regression-review.json
```

After all 3:
- All clean → proceed to Final Wave
- Issues found → spawn fixer (same as Audit Wave fix process)
