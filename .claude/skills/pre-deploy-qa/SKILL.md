...
In addition to feature-specific acceptance criteria, verify these fundamentals.
Populate this section with project-specific baseline checks from project-knowledge.
Generic categories to consider:

### Core Response Handling
- [ ] Primary user interaction path produces expected output
- [ ] Long output handled correctly (split, paginated, or truncated as designed)
- [ ] Output encoding (HTML, Markdown, plain text) does not crash on edge-case input

### File / Media Processing (if applicable)
- [ ] Supported file types accepted and processed correctly
- [ ] Oversized input → graceful error message (not crash)
- [ ] Unsupported file type → graceful rejection

### Stateful Features (if applicable)
- [ ] State reads and writes work end-to-end
- [ ] State display commands render correctly
- [ ] State mutation commands (toggle, clear) behave as specified

### Access Control / Subscription (if applicable)
- [ ] Each user tier receives the correct feature set
- [ ] Rate limits are enforced server-side
- [ ] Access checks do not crash on missing or expired entitlement records

### Error Resilience
- [ ] External API timeout → user receives an error message (not silence)
- [ ] External API error → fallback chain executes as designed
