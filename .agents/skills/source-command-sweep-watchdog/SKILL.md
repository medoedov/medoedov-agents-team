...
description: |
  Codex adapter for the Claude `/sweep-watchdog` command. Single sweep cycle for the watchdog agent. Invoked via /loop 10m. Reads
...
/sweep-watchdog` immediately after spawn_agent orchestration and continues until the
...
- For each active teammate: list_agents unread-message count and `next:` marker.
...
- **S1 — Multi-reviewer round desync.** `len(reviewers) >= 2` AND the
  teammate's last `send_message` to any reviewer is >30 min old AND the
...
- **S2 — Idle with pending inbox.** Teammate emitted `next: idle` while
  list_agents shows `has_unread == true`. Action: 3-ping protocol (Step 4).
...
- `consecutive_stale == 1`: `send_message(teammate, <Surface 1 ping text>)` —
  the Surface 1 template from the agent spec. Log entry with `pings_sent: 1`.
- `consecutive_stale == 2`: `send_message(teammate, <Surface 1 ping text>)`
...
3. Attempt `a supported runtime automation scheduled for min(reset_at - now, 1 hour)` —
   buffer already in `reset_at`, do not re-add. Best-effort early nudge only:
   the runtime hard-clamps a supported runtime automation to 1 hour, so it cannot span a
...
- **Self-skip:** the teammate literally named `watchdog` must not appear in
  ...
  list_agents alongside other teammates.
...
- **Already-paused run:** if `stall_state.active == true` from a prior M4
  ...
  by `SessionStart.sh` / a supported runtime automation, not by the sweep itself.
