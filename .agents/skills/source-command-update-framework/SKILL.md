---
name: source-command-update-framework
description: |
  Codex adapter for the Claude `/update-framework` command. Подтягивает последнюю версию агентного фреймворка из публичного OS-репозитория в этот проект
  Use when the user invokes that command or asks for the equivalent workflow.
---

<!-- Generated by sync_to_codex.py. Do not edit directly. Source: .claude/commands/update-framework.md. -->
...
# Command workflow: /update-framework

Treat Claude-only command syntax as workflow intent and use the available Codex tools.

## Executable entrypoint

Executable entrypoint: `python .claude/shared/scripts/sync_from_os.py`
