...
1. create a supervised `skill-test-{skill-name}` subagent group with `spawn_agent`
...
- Each runner: `Load Codex skill: skill="{tested-skill-name}"`
- Model: as specified in scenario file
- Use `parallel spawn_agent calls`

Spawn 1 baseline runner:
- Same task prompt
- Receives no skill to load
- Same model, same `parallel spawn_agent calls`
