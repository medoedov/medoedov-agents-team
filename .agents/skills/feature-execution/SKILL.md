...
0. Check `work/{feature}/logs/checkpoint.yml`:
   - `last_completed_wave > 0` → this is a resume after context compaction.
     ...
     recreate via spawn_agent orchestration. Skip to Phase 2 starting from `next_wave`.
...
6. Create team via spawn_agent orchestration
7. Spawn watchdog teammate — `task_name: "general-purpose"`, `model: "haiku"`, `team_name: "{team}"`.
   ...
   Prompt: "You are the watchdog agent for feature {feature}. Run `recurring 10-minute sweep-watchdog monitoring` to monitor progress. Feature dir: {feature_dir}"
...
3. For each task, spawn **teammate + reviewers** (if task has reviewers):
   ...
   **Teammate** — `task_name: "general-purpose"`, `model: "opus"`, `team_name: "{team}"`
   ...
   If the task requires user actions — send the instruction to team lead via send_message.
   ...
   **{reviewers_block}** — include only when task has reviewers (not `reviewers: none`):
   ...
   2. Send each reviewer via send_message: list of changed files + full diff output.
   ...
   **Each reviewer** (when present) — `task_name: "{reviewer_agent}"`, `model: "sonnet"`, `team_name: "{team}"`
   ...
   3. Send report path to teammate "{teammate_name}" via send_message
